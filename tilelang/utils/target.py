...
SUPPORTED_TARGETS: dict[str, str] = {
    ...
    "maca": "MetaXGPU target. Use dict options such as {'kind': 'maca'}.",
...
def determine_target(target: TargetLike | Literal["auto"] = "auto", return_object: bool = False) -> str | TargetConfig | Target:
    ...
    if target == "auto":
        ...
        if torch.version.hip is not None and check_hip_availability():
            ...
            # Check for CUDA, HIP and MACA availability
            ...
            if is_maca_available:
                ...
                raise ValueError("No CUDA or HIP or MPS or MACA available on this system.")
