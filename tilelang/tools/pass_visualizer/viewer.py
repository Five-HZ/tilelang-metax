...
def build_pass_data(path: str, factory: str | None, target: str, kwargs: dict[str, object], source: str) -> tuple[str, list[dict]]:
    ...
    if resolved_target.kind.name not in ["cuda", "maca"]:
        raise ValueError(f"Pass Visualizer currently supports only CUDA/MACA targets, got {resolved_target.kind.name!r}.")
