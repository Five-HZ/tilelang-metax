...
def test_default_language_is_static_maca_facade():
    from tilelang.maca import language as maca_language

    assert tilelang.language is default_language
    assert importlib.import_module("tilelang.language") is default_language
    assert default_language.__tilelang_dialect__ == "maca"
    assert maca_language.__tilelang_dialect__ == "maca"
    assert set(default_language.__all__) == set(maca_language.__all__)

    for name in default_language.__all__:
        assert getattr(default_language, name) is getattr(maca_language, name)
