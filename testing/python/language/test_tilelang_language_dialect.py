...
def test_default_language_is_static_maca_facade():
    ...
    assert tilelang.language is T_default
    assert importlib.import_module("tilelang.language") is T_default
    assert T_default.__tilelang_dialect__ == "maca"
    assert maca_language.__tilelang_dialect__ == "maca"
    assert set(T_default.__all__) == set(maca_language.__all__)

    for name in T_default.__all__:
        assert getattr(T_default, name) is getattr(maca_language, name)
